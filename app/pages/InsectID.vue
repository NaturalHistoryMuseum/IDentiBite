  ...
  <Page>
    <ScrollView orientation="horizontal">
      <StackLayout orientation="horizontal" class="list-group" height="200">
        <GridLayout
          @tap="onInsectTap({item})"
          v-for="item in insectList"
          v-bind:key="item"
          :columns="insectList.length"
          width="100"
          height="150"
          backgroundColor="lightgray"
          rows="*, 50"
        >
          <Image height="100" width="100" :src="item.images[0] | imageAssetPath" row="0" />
          <Label :text="item.common_name" class="nameLabel" row="1"></Label>
        </GridLayout>
      </StackLayout>
    </ScrollView>
  </Page>
